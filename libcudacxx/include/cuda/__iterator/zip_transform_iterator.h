...
public:
  ...
  [[nodiscard]] _CCCL_API constexpr reference operator*() const
    noexcept(::cuda::std::is_nothrow_invocable_v<_Fn&, ::cuda::std::iter_reference_t<const _Iterators>...>)
  {
    return ::cuda::std::apply(__zip_op_star{const_cast<_Fn&>(*__func_)}, __current_);
  }

  struct __zip_op_subscript
  {
  ...
  _CCCL_API constexpr reference operator[](difference_type __n) const
    noexcept(noexcept(::cuda::std::apply(__zip_op_subscript{__n, const_cast<_Fn&>(*__func_)}, __current_)))
  {
    return ::cuda::std::apply(__zip_op_subscript{__n, const_cast<_Fn&>(*__func_)}, __current_);
  }
...
template <class _Fn, class... _Iterators>
struct _IsFancyPointer<::cuda::zip_transform_iterator<_Fn, _Iterators...>> : false_type
{};
